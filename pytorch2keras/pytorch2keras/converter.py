...
def pytorch_to_keras(input_shape, pytorch_output, change_ordering=False):
    ...
    # K.set_image_data_format('channels_first') # keras2.0
    K.set_image_dim_ordering('th') # keras1.2
    ...
    #Keras1.2 case
    model = keras.models.Model(layers['input'], output)
            ...
            if layer['config'] and 'batch_input_shape' in layer['config']:
                layer['config']['batch_input_shape'] = \
                    tuple(np.reshape(
                        [
                            None,
                            # *layer['config']['batch_input_shape'][2:][:], # python3case
                            layer['config']['batch_input_shape'][2],        # python2case
                            layer['config']['batch_input_shape'][3],        # python2case
