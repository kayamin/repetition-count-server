...
def convert_dense(node, node_name, input_name, output_name, layers):
    ...
    dense = keras.layers.Dense(
        output_channels,
        weights=weights, bias=has_bias, name=output_name
    )
...
def convert_convolution(node, node_name, input_name, output_name, layers):
    ...
    # Extract weights and biases values from pytorch model
    ...
    if len(weights_var.data.numpy().shape) == 4:
        ...
        if node.transposed:
            ...
            conv = keras.layers.Convolution2D(
                ...
                height,
                width,
                subsample=(node.stride[0], node.stride[1]),
